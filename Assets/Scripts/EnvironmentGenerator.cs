...
public class EnvironmentGenerator : MonoBehaviour
{
    ...
    public int maxSpawnDistance = 12;
    ...
    void RemoveDeadEnds()
    {
        List<Vector2Int> deadEnds = FindDeadEndPoints();
        
        foreach (var deadEnd in deadEnds)
        {
            int depth = MeasureDeadEndDepth(deadEnd.x, deadEnd.y);
            
            if (depth > 1)
            {
                BreakDeadEndWall(deadEnd.x, deadEnd.y);
            }
        }
    }

    List<Vector2Int> FindDeadEndPoints()
    {
    ...
    int MeasureDeadEndDepth(int startX, int startY)
    {
        int depth = 0;
        int currentX = startX;
        int currentY = startY;
        ...
        while (true)
        {
            ...
            if (maze[currentX + 1, currentY] == 0 && !(currentX + 1 == prevX && currentY == prevY))
                neighbors.Add(new Vector2Int(currentX + 1, currentY));
            if (maze[currentX - 1, currentY] == 0 && !(currentX - 1 == prevX && currentY == prevY))
                neighbors.Add(new Vector2Int(currentX - 1, currentY));
    ...
    void BreakDeadEndWall(int x, int y)
    {
        ...
        foreach (var dir in directions)
        {
            ...
            if (wallX > 0 && wallX < width - 1 && wallY > 0 && wallY < height - 1 &&
                ...
                maze[beyondX, beyondY] == 0)
            {
                breakableWalls.Add(new Vector2Int(wallX, wallY));
            }
        }
